	...
	public function accessRules() {
        ...
        public function actionIndex() {
            ...
            if(Yii::app()->user->roles == 'athlete'){
                ...
                left join `user` u on te.id_user = u.id where te.id_visibility != 3 and 
                        te.id_user in (select id_partner from sparring where id_user='.Yii::app()->user->id.' and status = 2) order by te.date DESC limit 8
                 ';
        ...
        public function actionLoadnews(){
            if(isset($_GET['offset'])){
                ...
                $sqlStatement = 'select te.*, u.fullname, u.profile_picture, u.gender from `training_entry` te 
                    left join `user` u on te.id_user = u.id where te.id_visibility != 3 and 
                        te.id_user in (select id_partner from sparring where id_user='.Yii::app()->user->id.' and status = 2)
                            order by te.date DESC limit 8 offset '.$_GET['offset'];
                }  elseif(Yii::app()->user->roles == 'coach') {
                    $sqlStatement = 'select te.*, u.fullname, u.profile_picture, u.gender from `training_entry` te 
                        left join `user` u on te.id_user = u.id where 
